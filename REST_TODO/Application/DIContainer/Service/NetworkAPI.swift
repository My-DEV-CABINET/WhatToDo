...
enum NetworkAPI {
    ...
    case postTodo(id: Int, dto: ToDoPOSTDTO) // POST - 데이터 추가
    case putTodo(id: Int) // PUT - 데이터 수정
    ...
    var param: [URLQueryItem] {
        ...
        case .requestTodoWithID(id: let id), .putTodo(let id), .deleteTodo(let id):
            return [
                URLQueryItem(name: "id", value: id.description)
            ]
        case .postTodo(let id, let dto):
            return [
                URLQueryItem(name: "id", value: id.description),
